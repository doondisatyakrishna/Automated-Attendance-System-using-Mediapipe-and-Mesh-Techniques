...
import { getTodaysAttendance } from '../api'; 
...
const CameraFeed: React.FC<{ onStudentRecognized: (student: any) => void; isCameraActive: boolean; setIsCameraActive: (isActive: boolean) => void; }> = ({ onStudentRecognized, isCameraActive, setIsCameraActive }) => {
    ...
    const startCamera = async () => {
        try {
            ...
            toast.info("Camera started. Recognition is active.");
        } catch (err) {
    ...
    const stopCamera = () => {
        ...
        if (streamRef.current) {
            ...
            if(isCameraActive) toast.info("Camera stopped."); // Only show toast if it was active
        }
    };
        ...
        canvas.toBlob(async (blob) => {
            ...
            try {
                ...
                if (res.status === 'match_found') {
                    onStudentRecognized(res);
                }
            } catch (e) { /* Ignore no match errors */ }
        }, 'image/jpeg');
    };
...
const TakeAttendancePage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchInitialLog = async () => {
            try {
                ...
                toast.error("Could not load today's attendance log.");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleStudentRecognized = (recognizedStudent: any) => {
        if (todaysLog.some(entry => entry.student_id === recognizedStudent.student_id)) {
            return; 
        }

        if (recognizedStudent.record) {
            setTodaysLog(prevLog => [recognizedStudent.record, ...prevLog]);
            toast.success(`${recognizedStudent.name} marked ${recognizedStudent.record.status}!`);
        }
    };
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-surface p-6 rounded-xl shadow-sm border border-border">
                <h2 className="text-lg font-semibold text-text-primary mb-4">Recognized Today ({todaysLog.length})</h2>
                {isLoading ? (
                    ...
                    <div className="max-h-[70vh] overflow-y-auto pr-2 space-y-3">
                        {todaysLog.length > 0 ? todaysLog.map((entry) => (
                            <div key={entry.student_id} className="flex items-center justify-between p-3 bg-background rounded-lg animate-fade-in-down">
                                <div className="flex items-center gap-3">
                                    {entry.photo_url ? (
                                        <img src={`http://localhost:8000${entry.photo_url}`} alt={entry.name} className="h-10 w-10 rounded-full object-cover"/>
                                    ) : (
                                        <div className="h-10 w-10 rounded-full bg-background border border-border flex items-center justify-center"><UserIcon className="h-6 w-6 text-text-secondary"/></div>
                                    )}
                                    <div>
                                ...
                                <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${entry.status === 'late' ? 'bg-yellow-500/10 text-yellow-600' : 'bg-primary/10 text-primary'}`}>
                                    {entry.status}
                                </span>
                            </div>
                        )) : (
