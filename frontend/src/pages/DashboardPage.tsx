...
import { getDashboardSummary, updateAttendanceStatus } from '../api';
...
const DashboardPage: React.FC = () => {
    ...
    const fetchData = async () => {
        try {
            // We don't set loading(true) here if it's a background refresh
    ...
    const handleStatusChange = async (studentId: string, newStatus: string) => {
        ...
        setLog(currentLog =>
            currentLog.map(entry =>
                entry.student_id === studentId ? { ...entry, status: newStatus as 'present' | 'late' | 'absent' } : entry
            )
        );

        try {
            const todayStr = new Date().toISOString().split('T')[0];
            await updateAttendanceStatus(studentId, todayStr, newStatus);
            toast.success(`Status updated to ${newStatus}`);
            // Refresh stats in background to update charts/counters
            fetchData(); 
        } catch (error) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                <div className="lg:col-span-3 bg-surface p-6 rounded-xl border border-border">
                    ...
                    {log.length > 0 ? (
                        <div className="max-h-[400px] overflow-y-auto pr-2 space-y-3">
                            {log.map(entry => (
                                <div key={entry.student_id} className="flex items-center justify-between p-3 bg-background rounded-lg transition-all hover:shadow-sm">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-text-secondary">
                                                {entry.timestamp
                                                    ? `Checked in at ${new Date(entry.timestamp).toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit', hour12: true })}`
                                                    : 'Not checked in'
                                                }
                                            </p>
                                        </div>
                                    ...
                                    <select
                                        value={entry.status}
                                        onChange={(e) => handleStatusChange(entry.student_id, e.target.value)}
                                        className={`px-2.5 py-1 text-xs font-semibold rounded-full border-none cursor-pointer focus:ring-2 focus:ring-primary/50 outline-none transition-colors appearance-none text-center
                                            ${entry.status === 'present' ? 'bg-primary/10 text-primary hover:bg-primary/20' : ''}
                                            ${entry.status === 'late' ? 'bg-yellow-500/10 text-yellow-600 hover:bg-yellow-500/20' : ''}
                                            ${entry.status === 'absent' ? 'bg-red-500/10 text-red-600 hover:bg-red-500/20' : ''}
                                        `}
                                    >
                ...
                <div className="lg:col-span-2 bg-surface p-6 rounded-xl border border-border">
                    ...
                    <div style={{ width: '100%', height: 300 }}>
                        <ResponsiveContainer>
                            <PieChart>
                                <Pie data={chartData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={70} outerRadius={90} paddingAngle={5} labelLine={false}>
